...
async function bridgeSolArb(
  recieverAddress: any,
  recieverTokenAccount: string,
  amount: number,
) {
  const wallet = await db
    .select()
    .from(userWallets)
    .where(eq(userWallets.address, recieverAddress))
    .execute();
...
const cctpSolArb = async (
  srcWallet: WalletMeta,
  destWallet: WalletMeta,
  quantity: number,
  tokenAccount: string,
) => {
  const provider = new anchor.AnchorProvider(
    ...
    new anchor.Wallet(
      Keypair.fromSecretKey(bs58.decode(srcWallet.private_key ?? "")),
    ),
    {},
  );

  const payerProvider = new anchor.AnchorProvider(
    new Connection(
      process.env.SOLANA_RPC_URL ?? "https://api.mainnet-beta.solana.com",
    ),
    new anchor.Wallet(
      Keypair.fromSecretKey(
        bs58.decode(process.env.SOLANA_PAYER_PRIVATE_KEY ?? ""),
      ),
    ),
    {},
  );
  ...
  const mintRecipient = new PublicKey(
    getBytes(evmAddressToBytes32(destWallet.address)),
  );
  ...
  const depositForBurnTx = await provider.sendAndConfirm(
    finalTx,
    [
  ...
  const attestationHex = (xfer.attestations?.at(0)?.attestation as Attestation)[
    "attestation"
  ];
  ...
  await db
    ...
    .where(
      and(
        eq(bridges.sourceTxId, depositForBurnTx),
        eq(bridges.userId, srcWallet.userId),
      ),
    )
    .execute();
  ...
  payerProvider
    .sendAndConfirm(reclaimEventAccountTxUnsigned, [
      Keypair.fromSecretKey(
        bs58.decode(process.env.SOLANA_PAYER_PRIVATE_KEY ?? ""),
      ),
    ])
    .then((reclaimEventAccountTx) => {
