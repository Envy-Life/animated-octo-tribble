...
async function bridgeHL(wallet: any, amount: number) {
  ...
  let etherWallet = new ethers.Wallet(
    wallet[0].private_key ?? "",
    new ethers.JsonRpcProvider(process.env.ARBITRUM_RPC_URL),
  );
  ...
  let tx = await contract.batchedDepositWithPermit(
    [[wallet[0].address, amount, deadline, [data.r, data.s, data.v]]],
    {
      gasLimit: 1000000,
