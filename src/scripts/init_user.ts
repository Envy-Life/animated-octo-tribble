...
async function rollbackSolWebhooks(address: string) {
    await helius.removeAddressesFromWebhook(
        process.env.HELIUS_WEBHOOK_ID ?? '',
        [address],
    )
    console.log(`Webhook updated for address: ${address}`);
...
async function updateArbWebhooks(address: string) {
    ...
    await fetch('https://api.quicknode.com/kv/rest/v1/lists/walletStore', requestOptions)
...
async function rollbackArbWebhooks(address: string) {
    ...
    await fetch('https://api.quicknode.com/kv/rest/v1/lists/{key}', requestOptions)
...
async function rollback(ctx: Scenes.WizardContext<BotWizardSession>) {
    await db.transaction(async (tx) => {
        ...
        await tx.delete(schema.users).where(
            sql`${schema.users.userId} = ${ctx.from?.id ? ctx.from.id.toString() : ''}`
        );
    }
    )
}
